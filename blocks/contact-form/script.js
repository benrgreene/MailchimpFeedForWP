(() => {
  const blockName = 'mailchimp/contact-form'
  ...
  const loadEvents = (attributes) => {
    const newsletterListWrappers = document.querySelectorAll('[data-get-mc-contact-form]')
    newsletterListWrappers.forEach((wrapper) => {
      fetch(`${window.wpApiSettings.root}mailchimp/contact-form-render`)
        .then((blob) => blob.text())
        .then((data) => {
